from block_sban import ConvVSSNetwork
...
class CGDMamba(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2],
                 dims=[96, 192, 384, 768], d_state=16, drop_rate=0.1, attn_drop_rate=0.1, drop_path_rate=0.2,
    ...
    # def forward(self, x):
    #     # 这里由于要使用渐进式训练，所以只需要抽取特征，不要进行head分类计算损失
    #     x = self.patch_embed(x)
